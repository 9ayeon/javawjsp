...
<head>
  ...
  <script>
      ...
      let homePage = myform.homePage.value;
  		...
  		let ext = fName.substring(fName.lastIndexOf(".")+1);  // .뒤에 확장자 가져오기
  		let uExt = ext.toUpperCase(); // 확장자를 대문자로 변경함.
      ...
      if(tel2 != "" || tel3 != "") {
  		...
  		// 파일 전송 전에 파일에 관한 사항 체크(파일명이 넘어올 경우는 해당 파일을 넘기고, 비었으면 'noimage.jpg'를 넘겨준다.)
  		if(fName.trim() == "") {
  			...
  			submitFlag = 1;
				...
				if(fileSize > maxSize) {
					...
					alert("업로드 파일의 확장자는 'JPG/GIF/PNG' 입니다. ");
					return false;
				}
  		...
  		// 전송 전에 모든 체크가 끝났다면 submitFlag가 1이 되도록 처리후 서버로 전ㅅ농한다.
  		if(submitFlag == 1) { // 1이면 정상
    		...
    		alert("회원가입 실패~~");
  			...
  			/* 	// 이메일을 하나로 묶어준다.
  				...
  				myform.email.value = email1 + "@" + email2;
					...
					let detailAddress = myform.detailAddress.value + " ";
					let extraAddress = myform.extraAddress.value + " ";
					myform.address.value = postc ode + "/" + roadAddress + "/" + detailAddress + "/" + extraAddress;*/
