...
<head>
  ...
  <script>
    'use strict';
    // 닉네임 중복버튼을 클릭했는지를 확인하기위한 전역변수를 정의한다.(버튼클릭후에도 내용을 수정했다면 초기값은 0으로 셋팅해서 버튼을 누를수 있도록해야한다.)
  	let nickCheckSw = 0;
    ...
    function fCheck() {
  	...
  	// 전송전에 모든 체크가 끝나서 submitFlag가 1이되면 서버로 전송한다.
    	if(submitFlag == 1) {
    		...
    		if(nickName == '${sNickName}') { // (고치지않았으면)기존의 닉네임과 입력값이 같으면 1로 처리(전송)
    			nickCheckSw = 1;
    		}
  			if(nickCheckSw == 0) { // 다를시 중복체크해야하니까 0으로 처리.
      ...
      <label for="email1">Email address:</label>
				<div class="input-group mb-3">
				  ...
				  <div class="input-group-append">
				    <select name="email2" class="custom-select"><!-- if email2가 naver.com이면 이걸 셀렉티드해서 네이버가 선택됨. -->
      ...
      <div class="input-group mb-3">
	      <div class="input-group-prepend">
	        <span class="input-group-text">전화번호 :</span> &nbsp;&nbsp;
			      <select name="tel1" class="custom-select"><!-- 삼항연산자 이용해서수정처리 -->
