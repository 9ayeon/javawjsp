...
%>
  	...
  	1-3. 사용예 : 1234.567 => <fmt:formatNumber value="1234.567" pattern="#,##0.0" /><br/>
		2-1. 화폐단위 : <fmt:formatNumber value="${won}" type="currency" /><br/> <!-- currency = 국가별로 -->
  	...
  	<p>자바 형식의 날짜를 jstl변수에 저장 : <c:set var="now" value="<%=new java.util.Date()%>"></c:set></p>
  	<p>
  		오늘 날짜: ${now} <br/>
  	...
  	<p>
  		...
  		톰캣 서버의 기본 로케일(미국식 변환) : <fmt:setLocale value="en_US"/><br/>
  		톰캣 서버의 기본 로케일(일본식 변환) : <fmt:setLocale value="ja_JP"/><br/>
  		<fmt:formatNumber value="${won}" type="currency"/><br/> <!-- 마지막으로 변환한 나라 화폐단위로 출력됨 -->
  	</p>
  	<h4>기타</h4>
  	<p>지정된 곳으로 이동: location.href()</p>
  	<%-- <c:redirect url="jstl1.jsp"/> f11실행하면 바로 jstl1이 실행됨--%>
