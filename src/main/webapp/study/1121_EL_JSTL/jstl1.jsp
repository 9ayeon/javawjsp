...
<div class="container">
  ...
  <p>사용예:<br/> <!-- 값 저장 후 el로 출력 -->
  	su1변수선언 후 초기값을 100으로 할당 <c:set var="su1" value="100" /><br/>
  	su1변수의 값을 출력 <c:out value="${su1}"/> == ${su1} <br/> <!-- su1의 값이 su1안에 들어있으니까 -->
  </p>
  <hr/>
  ...
  1-2. su1 != su2 : <c:if test="${su1 != su2}">su1과 su2는 다르다.</c:if><br/><br/>
  2-1. su1 >= su2 : <c:if test="${su1 >= su2}">su1과 su2보다 크거나 같다.</c:if><br/>
  2-2. su1 < su2 : <c:if test="${su1 < su2}">su1은 su2보다 작다.</c:if><br/><br/> <!-- 배타적?으로비교 -->
  3-1. su2 == su3 : <c:if test="${su2 == su3}">su2 과 su3는 같다.</c:if><br/> 
  3-2. su2 != su3 : <c:if test="${su2 != su3}">su2 과 su3는 다르다.</c:if><br/><br/> 
  <!-- 숫자로 비교하려면 먼저 연산을 해준다.(값에 들어있는 공백을 없애고나서) -->
 <%--  4-1. su2 == su3 : <c:if test="${su2+0 == su3+0}">su2 과 su3는 같다.</c:if><br/> 
  4-2. su2 != su3 : <c:if test="${su2+0 != su3+0}">su2 과 su3는 다르다.</c:if><br/>  --%>
	<hr/>
	<c:set var="jum" value="${param.jumsu}"/>
	<!--  elst if가 없어서 순서를 바꿔서 해야한다.( >=연산자 사용한 조건문은 낮은점수비교부터 ) -->
	...
	<c:if test="${param.gender == '1'}"><c:set var="gender" value="남자" /></c:if>
	<c:if test="${param.gender == '2'}"><c:set var="gender" value="여자" /></c:if>
	...
	<form name="myform">
		...
		<div>
			...
			<input type="submit" value="학점출력" class="btn btn-info" />
		</div>
		<div>
				입력한 점수 : <font color="red"><b>${jum}</b></font><br/> 
				계산된 학점 <font color="red"><b><span id="demo">${grade} 학점</b></font></span><br/>
				성별 : <font color="red"><b>${gender}</b></font><br/>
		</div>
	</form>
	<br/>
	<hr/>
