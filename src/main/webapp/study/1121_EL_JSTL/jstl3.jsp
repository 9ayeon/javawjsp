...
<%@page import="study.database.JusorokDAO"%> <!-- 맨뒤에 *쓰면 그 폴더 안에 권한 가져옴 -->
...
<div class="container">
  ...
  <table class="table table-hover">
  	...
  	<tr><td colspan="6" class="m-0 p-0"></td></tr> <!-- 맨아랫줄 선긋기 -->
  </table>
  <p>전체 출력 건수 : <font color="red"><b>${cnt0}</b></font>건</p>
  <hr/>
  <pre>
  	...
  	<table class="table table-hover">
	  	...
	  	<c:forEach var="vo" items="${vos}" step="2" varStatus="st">
		  	...
		  	<c:set var="cnt" value="${st.count}" /></p>
	  	</c:forEach>
	  	<tr><td colspan="6" class="m-0 p-0"></td></tr> <!-- 맨아랫줄 선긋기 -->
  	</table>
  	...
  	<table class="table table-hover">
	  	...
	  	<c:set var="cnt2" value="0"/> <!-- 누적하는방법, 83행,95행, 99번 -->
	  	<c:forEach var="vo" items="${vos}" varStatus="st">
  	...
  	<p>전체 출력 건수 : <font color="red"><b>${cnt2}</b></font>건</p>
  </pre>
</div>
<p><br/></p>
</body>
