...
<div class="container">
 ...
 <%
 	...
 	pageContext.setAttribute("atom", atom); // 한페이지에서만 유효
 ...
 atom = ${param.atom}<br/> <!-- form.jsp -->
 name = ${param.name}<br/>
 su1 = ${param.su1}<br/>
 su2 = ${param.su2}<br/>
 <h4>jsp에서 폼태그(get/post)로 넘긴 자료를 서블릿을 통해서 받을 때</h4>
 atom = ${vo.atom}<br/> <!-- 여러개일시 원래는 vo에 담는다 -->
 name = ${vo.name}<br/>
 su1 = ${vo.su1}<br/>
 su2 = ${vo.su2}<br/>
...
%>
 <h3>현재 폼에서의 값의 전달</h3>
 <div>
 	<form> <!-- 현재 폼에서 전달할 때는 폼네임과 메소드, 액션 생략 가능, 서버로 받는다(내가 보내고 내가 받는다.) -->
 		<div>아이디 : <input type="text" name="mid" value="${mid}"/></div>
 		<div><input type="submit" value="전송"/></div>
 		<div>전송된 아이디 : <font color="blue"><b>${mid}</b></font></div>
 	</form>
 </div>
</div>
<p><br/></p>
