...
@WebServlet(urlPatterns="/Green", initParams = {@WebInitParam(name="mid", value="admin"), @WebInitParam(name="pwd", value="1234"), @WebInitParam(name="className", value="총무과")})//url-pattern, 여기 적어주는 값으로 초기값할당해주겠다(web.xml과 유사) init(초기화)
public class GreenInit extends HttpServlet{
	@Override
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 위(webservlet)에 설정해둔 변수/값을 가져온다.
