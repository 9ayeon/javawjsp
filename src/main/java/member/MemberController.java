...
import admin.AdMemberDelCommand;
...
public class MemberController extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * // 세션이 끊겼다면 작업의 진행을 중지 시키고 홈으로 전송 시켜준다. HttpSession session =
		 * request.getSession(); int level = session.getAttribute("sLevel")== null ? 99
		 * : (int) session.getAttribute("sLevel");
		 * 
		 * if(level >= 4) { RequestDispatcher dispatcher =
		 * request.getRequestDispatcher("/"); // 우리사이트의루트로?....????...여튼,,,홈으로,,
