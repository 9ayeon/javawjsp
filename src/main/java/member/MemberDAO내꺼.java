...
public class MemberDAO내꺼 {
	...
	public MemberVO getLoginCheck(String mid) {
		...
		try {
			...
			System.out.println("SQL 오류0 : " + e.getMessage());
		} finally {
			getConn.rsClose(); //싱글톤에있는 객체를 통해서
		}
		return vo; // 자료가있으면 vo에 담긴다.
	}

	// 오늘 처음 방문시 방문카운트로 0으로 초기화
	public void setTodayCntUpdate(String mid) {
		try {
			...
			getConn.pstmtClose(); //싱글톤에있는 객체를 통해서
		}
	}
	
	//오늘 재방문이라면 '총밤문수','오늘방문수','포인트' 누적처리
	public void setMemTotalUpdate(String mid, int nowTodayPoint) {
		try {
			...
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, nowTodayPoint); // 1번재물음표
			pstmt.setString(2, mid); // 2번재물음표
			pstmt.executeUpdate();
		} catch (SQLException e) {
	...
	public ArrayList<MemberVO> getMemList(int startIndexNo, int pageSize, int level) {
		...
		try {
			if(level != 0) { // 관리자가 아니면 공개된 정보만.
				sql = "select * from member where userInfor = '공개' and mid like ? order by idx desc limit ?,?";
			}
			else { // 관리자는 모든 것을 볼 수 있음.
				sql = "select * from member where mid like ? order by idx desc limit ?,?";
			}
			pstmt = conn.prepareStatement(sql);//pstmt생성?씰행,,,
			pstmt.setString(1, "%"+mid+"%");
			pstmt.setInt(2, startIndexNo);
			pstmt.setInt(3, pageSize);
			//rs = pstmt.executeQuery(); // 그냥가져오는거심.,,
			
			while(rs.next()) {
				vo = new MemberVO(); //vo의 껍데기에 담을것,,,
				vo.setIdx(rs.getInt("idx"));
				vo.setMid(rs.getString("mid"));
				vo.setPwd(rs.getString("pwd"));
				vo.setNickName(rs.getString("nickName"));
	...
	// 회원 탈퇴 처리(userDel필드의 값을 'OK'로 변경처리한다.) , 로그인이 되지않게 처리해야한다.로그인커맨드객체로
